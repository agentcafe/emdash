...
- **Write-exclusive:** Path locks ensure only one write operation can operate on a path at a time
- **On by default:** All data operations automatically acquire locks; no extra configuration needed
- **Lock as protection:** LockContext acquires locks on entry, releases on exit — no undo/journal/commit semantics
- **Only session_memory needs crash recovery:** RedoLog re-executes memory extraction after a process crash
- **Queue operations run outside locks:** SemanticQueue/EmbeddingQueue enqueue operations are idempotent and retriable

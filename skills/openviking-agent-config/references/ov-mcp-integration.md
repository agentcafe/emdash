...
OpenViking can be used as an MCP (Model Context Protocol) server, allowing any MCP-compatible client to access its memory and resource capabilities.
...
The root cause is not a broken index — it is multiple processes contending for the same storage files. Switch to HTTP mode to resolve this. See Troubleshooting for details.

## Setup

### Prerequisites

- OpenViking installed (`pip install openviking` or from source)
- A valid configuration file (see Configuration Guide)
- For HTTP mode: `openviking-server` running (see Deployment Guide)
...
- Switch to HTTP mode to avoid contention.
- If using HTTP: reload the MCP connection in your client (restart the session or reconnect).
